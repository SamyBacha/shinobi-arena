...
class DuelScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    // Force le stage BOSS_GIF si combat contre le Magician en vsAI
    const isBossCandidate = data.p2 && data.p2.trait === 'magic_shield' && (data.vsAI || false);
    const bossGifIdx = GIF_SCENES.indexOf(BOSS_GIF);

    if (isBossCandidate) {
      // Toujours le stage boss, même si BOSS_GIF introuvable (fallback index 1)
      this.stageIndex = bossGifIdx !== -1 ? bossGifIdx + 1 : 1;
    } else if (data.stageIndex) {
      this.stageIndex = data.stageIndex;
    } else {
      this.stageIndex = Phaser.Math.Between(1, Math.max(1, GIF_SCENES.length));
    }
  }
  ...
  preload() {
    ...
    // FX sprites (individual 496x496 PNGs)
    const fxBase = 'img/slash-sprite-cartoon-effects/PNG/';
    const fxSets = {
      'fx_slash':       { folder: 'slash/',          count: 8 },
      'fx_slash_blue':  { folder: 'slash-blue/',     count: 8 },
      'fx_red_slash':   { folder: 'red-slash/',      count: 8 },
      'fx_blue_attack': { folder: 'blue-attack/',    count: 10 },
      'fx_green_slash': { folder: 'green-slash/',    count: 10 },
      'fx_special':     { folder: 'special-slash/',  count: 8 },
      'fx_smoke':       { folder: 'smoke/',          count: 10 },
      'fx_smoke2':      { folder: 'smoke2/',         count: 10 },
      'fx_flames':      { folder: 'yellow-flammes/', count: 10 },
    };
    Object.entries(fxSets).forEach(([key, info]) => {
      for (let i = 1; i <= info.count; i++) {
        const imgKey = key + '_' + i;
        if (!this.textures.exists(imgKey)) {
          this.load.image(imgKey, fxBase + info.folder + i + '.png');
        }
      }
    });

    if (!this.cache.audio.exists('duel_bgm')) {
      this.load.audio('duel_bgm', 'music/Daishō-Two-Swords.mp3');
    }
    if (!this.cache.audio.exists('duel_boss_bgm')) {
      this.load.audio('duel_boss_bgm', 'music/unrealmicikcz12-the-big-one-fighting-music-338746.mp3');
    }
    if (!this.cache.audio.exists('duel_gameover')) {
      this.load.audio('duel_gameover', 'music/Game Over sound effect.mp3');
    }
    if (!this.cache.audio.exists('duel_samurai')) {
      this.load.audio('duel_samurai', 'music/Samurai Warriors Music The End (Game Over).mp3');
    }
    if (!this.cache.audio.exists('duel_sword')) {
      this.load.audio('duel_sword', 'music/dragon-studio-sword-slice-393847.mp3');
    }
    if (!this.cache.audio.exists('duel_punch')) {
      this.load.audio('duel_punch', 'music/universfield-classic-punch-impact-352711.mp3');
    }
    if (!this.cache.audio.exists('boss_useless')) {
      this.load.audio('boss_useless', 'voices/boss_useless.mp3');
    }
    if (!this.cache.audio.exists('onre_counter')) {
      this.load.audio('onre_counter', 'voices/Onre_counter.mp3');
    }
    if (!this.cache.audio.exists('peasant_special')) {
      this.load.audio('peasant_special', 'voices/Peasant_special_ability.mp3');
    }

    // Character voices — fichiers optionnels dans voices/{folder}_{event}.mp3
    [this.p1Def, this.p2Def].forEach(charDef => {
      const f = charDef.folder.replace(/\s+/g, '_');
      ['intro', 'win', 'lose'].forEach(ev => {
        const key = 'voice_' + f + '_' + ev;
        if (!this.cache.audio.exists(key)) {
          this.load.audio(key, 'voices/' + f + '_' + ev + '.mp3');
        }
      });
    });

    // Stage background + foreground layers
    // Tous les stages sont des GIFs dans img/world/scenes/
    // Chargés comme <img> DOM dans create(), pas via Phaser
  }
  ...
  create() {
    ...
    // Détection boss final : Magician (trait magic_shield) en vsAI OU stage BOSS_GIF sélectionné
    const bossGifIdx = GIF_SCENES.indexOf(BOSS_GIF);
    this.isBossFight = (this.p2Def.trait === 'magic_shield' && this.vsAI) ||
      (bossGifIdx !== -1 && this.stageIndex === bossGifIdx + 1);

    // Fond transparent — le GIF DOM est visible derrière le canvas
    this.cameras.main.setBackgroundColor('rgba(0,0,0,0)');


    // Music — boss BGM si boss fight (inclut stage 9), sinon BGM normal
    const bgmKey = this.isBossFight ? 'duel_boss_bgm' : 'duel_bgm';
    ...
    // Stage background
    {
      // Tous les stages sont des GIFs — <img> DOM derrière le canvas Phaser
      const gifFilename = GIF_SCENES[this.stageIndex - 1] || GIF_SCENES[0];
      const gameCanvas = this.sys.game.canvas;
      const rect = gameCanvas.getBoundingClientRect();

      this._gifImg = document.createElement('img');
      this._gifImg.src = 'img/world/scenes/' + gifFilename;
    ...
    this.p1MaxLives = DUEL_MAX_LIVES + (this.p1Def.trait === 'extra_life' ? 1 : 0);
    this.p2MaxLives = DUEL_MAX_LIVES + (this.p2Def.trait === 'extra_life' ? 1 : 0);
    ...
    this.p1Sprite.setScale(DUEL_PLAYER_SCALE * (FRAME_SIZE / (this.p1Def.frameSize || FRAME_SIZE)) * (this.p1Def.duelScale || 1));
    ...
    this.p2Sprite.setScale(DUEL_PLAYER_SCALE * (FRAME_SIZE / (this.p2Def.frameSize || FRAME_SIZE)) * (this.p2Def.duelScale || 1));
  ...
  createFxAnims() {
    ...
    makeFxAnim('fx_anim_slash',       'fx_slash',       8,  18);
    makeFxAnim('fx_anim_slash_blue',  'fx_slash_blue',  8,  18);
    makeFxAnim('fx_anim_red_slash',   'fx_red_slash',   8,  18);
    makeFxAnim('fx_anim_blue_attack', 'fx_blue_attack', 10, 16);
    makeFxAnim('fx_anim_green_slash', 'fx_green_slash', 10, 16);
    makeFxAnim('fx_anim_special',     'fx_special',     8,  16);
    makeFxAnim('fx_anim_smoke',       'fx_smoke',       10, 14);
    makeFxAnim('fx_anim_smoke2',      'fx_smoke2',      10, 14);
    makeFxAnim('fx_anim_flames',      'fx_flames',      10, 14);
  }
  ...
  playResolveAnimations(a1, a2) {
    ...
    if (f1) {
      ...
      const p1Sfx = this.p1Def.folder === 'Fighter' ? 'duel_punch' : 'duel_sword';
      ...
      if (lvl1 >= 2) {
        ...
        this.playFx('fx_anim_blue_attack', p1StartX, p1y, 0.5, false, 150, rush + 100);
      }
    } else if (a1 === P) {
    ...
    if (f2) {
      ...
      const p2Sfx = this.p2Def.folder === 'Fighter' ? 'duel_punch' : 'duel_sword';
      ...
      if (lvl2 >= 2) {
        ...
        this.playFx('fx_anim_blue_attack', p2StartX, p2y, 0.5, true, 150, rush + 100);
      }
    } else if (a2 === P) {
      // Flight trait: use flight anim instead of shield
    ...
    if (this._p1DisguiseAttack) {
      ...
      this.playFx('fx_anim_slash', p2StartX, p2y, 0.45, true, 200, disguiseDelay + 300);
    ...
    if (this._p2DisguiseAttack) {
      ...
      this.playFx('fx_anim_slash', p1StartX, p1y, 0.45, false, 200, disguiseDelay + 300);
      this.time.delayedCall(disguiseDelay + 300, () => {
    ...
    if (p2Hit) {
      if (lvl1 === 1) {
        this.playFx('fx_anim_slash', p2StartX, p2y, 0.45, true, 200, hurtDelay);
      } else if (lvl1 === 2) {
        this.playFx('fx_anim_slash_blue', p2StartX, p2y, 0.5, true, 200, hurtDelay);
        this.playFx('fx_anim_flames', p2StartX, p2y + 20, 0.4, false, 199, hurtDelay + 100);
      } else if (lvl1 >= 3) {
        this.playFx('fx_anim_slash_blue', p2StartX - 20, p2y - 20, 0.45, true, 200, hurtDelay);
        this.playFx('fx_anim_red_slash', p2StartX + 20, p2y, 0.5, true, 201, hurtDelay + 80);
        this.playFx('fx_anim_special', p2StartX, p2y, 0.55, true, 202, hurtDelay + 150);
        this.playFx('fx_anim_flames', p2StartX, p2y + 20, 0.5, false, 199, hurtDelay + 50);
        if (lvl1 >= 4) {
          this.playFx('fx_anim_special', p2StartX - 20, p2y - 10, 0.6, true, 203, hurtDelay + 250);
          this.playFx('fx_anim_flames', p2StartX + 10, p2y, 0.55, false, 204, hurtDelay + 300);
        }
      }
    }
    if (p1Hit) {
      if (lvl2 === 1) {
        this.playFx('fx_anim_slash', p1StartX, p1y, 0.45, false, 200, hurtDelay);
      } else if (lvl2 === 2) {
        this.playFx('fx_anim_slash_blue', p1StartX, p1y, 0.5, false, 200, hurtDelay);
        this.playFx('fx_anim_flames', p1StartX, p1y + 20, 0.4, false, 199, hurtDelay + 100);
      } else if (lvl2 >= 3) {
        this.playFx('fx_anim_slash_blue', p1StartX + 20, p1y - 20, 0.45, false, 200, hurtDelay);
        this.playFx('fx_anim_red_slash', p1StartX - 20, p1y, 0.5, false, 201, hurtDelay + 80);
        this.playFx('fx_anim_special', p1StartX, p1y, 0.55, false, 202, hurtDelay + 150);
        this.playFx('fx_anim_flames', p1StartX, p1y + 20, 0.5, false, 199, hurtDelay + 50);
        if (lvl2 >= 4) {
          this.playFx('fx_anim_special', p1StartX + 20, p1y - 10, 0.6, false, 203, hurtDelay + 250);
          this.playFx('fx_anim_flames', p1StartX - 10, p1y, 0.55, false, 204, hurtDelay + 300);
        }
      }
    }
    ...
    if (p1FlightCounter) {
      ...
      this.playFx('fx_anim_slash', p2StartX, p2y, 0.45, true, 200, counterDelay + walkDuration + 200);
    ...
    if (p2FlightCounter) {
      ...
      this.playFx('fx_anim_slash', p1StartX, p1y, 0.45, false, 200, counterDelay + walkDuration + 200);
      this.time.delayedCall(counterDelay + walkDuration + 200, () => {
        if (this.p1.lives > 0) this.p1Sprite.play('duel_p1_hurt');
        else this.p1Sprite.play('duel_p1_dead');
      });
  ...
  showPauseChars() {
    ...
    allChars.forEach(charDef => {
      ...
      if (this.textures.exists(spriteKey)) {
        ...
        spr.setScale(0.3 * (FRAME_SIZE / fs));
        spr.setDepth(611);
