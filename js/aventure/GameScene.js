...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    Object.keys(sheets).forEach(key => {
      this.load.spritesheet(key, this.charDef.assetFolder + sheets[key].file, {
        frameWidth: this.charDef.frameSize,
        frameHeight: this.charDef.frameSize
      });
    });

    // Ghost enemy sheets
    Object.values(GHOST_CHARACTERS).forEach(enemy => {
      const eBase = enemy.assetFolder;
      Object.keys(enemy.sheets).forEach(key => {
        ...
        this.load.spritesheet(sheetKey, eBase + enemy.sheets[key].file, {
          frameWidth: enemy.frameSize,
          frameHeight: enemy.frameSize
        });
      });
    });
  ...
  populateChunkEnemies(startX, chunkIndex, chunkData) {
    ...
    const ghostTypes = Object.values(GHOST_CHARACTERS);
